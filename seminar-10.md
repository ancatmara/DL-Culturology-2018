...
Осталось настроить футер. Добавляем туда контакты. Делаем адрес электронной почты кликабельным \(для этого в адресе ссылки перед почтой пишем _mailto:_\).

Сохраняем результат \(справа вверху есть специальная кнопка\). Перед публикацией обязательно надо проверить и настроить, если надо, мобильную версию \(она независима, там тоже всё можно менять\).
